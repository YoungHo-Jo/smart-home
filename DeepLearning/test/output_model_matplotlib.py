...
def draw_PlugState(plugs_data) : 
    ...
    for col_name in plugs_data.columns :
        plugs.append(list(map(float,plugs_data[col_name].values.tolist())))
    # print(timestep)
...
def Agglomerative(dataSet, n_clusters = 4, draw = False) :
...
#     dataset_plugs = dataset.fillna(0)
    ...
    list_plugs_kalman_T = KalmanFilter(list_plugs_T)
    # # list_plugs_kalman_T = KalmanFilter(list_plugs_kalman_T, Q_var = 0.01, R_var = 0.2,draw=True)

    list_plugs_kalman = list(map(list, zip(*list_plugs_kalman_T)))

    list_plugs_cluster = Agglomerative(list_plugs_kalman, draw = True)
